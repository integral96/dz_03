...
void print_ip(Arg&& arg) {
    if constexpr(std::is_same_v<Arg, int8_t> || std::is_same_v<Arg, int16_t>) {
        int16_t sdc = static_cast<unsigned char>(std::forward<Arg>(arg));
        std::cout << sdc;
    } else if constexpr(std::is_same_v<Arg, int32_t>) {
    ...
    } else if constexpr(std::is_same_v<Arg, std::vector<int>> || std::is_same_v<Arg, std::list<short>>) {
    ...
    } else if constexpr(std::is_same_v<Arg, std::tuple<int, int, int, int>>) {
        const auto& vec = std::forward<Arg>(arg);
        std::cout << std::get<0>(vec) << '.' << std::get<1>(vec)
             << '.' << std::get<2>(vec) << '.' << std::get<3>(vec);
    } else if constexpr(std::is_same_v<Arg, my_tuple<int, int, int, int>>) {
        const auto& vec = std::forward<Arg>(arg);
